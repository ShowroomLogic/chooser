...
chooserDemo.controller('multipleAdvancedCtrl', function($scope, $timeout) {
	...
	$scope.$watchCollection('[options, model]', function(values) {
		...
		if (model) {
			...
			for (i=0; i < model.length; i++) {
				if (!_.find(options, {value:model[i]})) {
					$scope.isValid = false;
					break;
				}
			}
		...
		if (options == null) {
			$scope.placeholder = "Loading Options...";
		} else if (!$scope.model || $scope.model == []) {
			$scope.placeholder = "Add Item";
		} else {
